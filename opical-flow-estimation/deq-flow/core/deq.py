...
class DEQFlowBase(nn.Module):
    def __init__(self):
        ...
        # else:
        self.fnet = BasicEncoder(output_dim=odim, norm_fn='instance', dropout=0.0)        
        self.cnet = BasicEncoder(output_dim=cdim, norm_fn='batch', dropout=0.0)
        ...
        self.f_thres = 36
        self.eval_f_thres = int(self.f_thres * 1.5)
        self.stop_mode = "abs"
...
class DEQFlowIndexing(DEQFlowBase):
    def __init__(self):
        super(DEQFlowIndexing, self).__init__()
        ...
        indexing_pg = make_pair(self.indexing, [1])
        produce_grad = [
                backward_factory(grad_type=pg, tau=1.0, sup_all=True) for pg in indexing_pg
                ]
        # Enabling args.ift will replace the last gradient function by IFT.
        produce_grad[-1] = backward_factory(
            grad_type='ift', safe_ift=True, b_solver=eval('broyden'),
            b_solver_kwargs=dict(threshold=36, stop_mode="abs")
            )
