...
try:
    from liteflownet.correlation import correlation # the custom cost volume layer
except:
...
class LiteFlowNet(torch.nn.Module):
    def __init__(self, load_pretrained=True, weights_path=None, rgb2bgr=False):
        ...
        if load_pretrained:
            self.net.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.hub.load_state_dict_from_url(url='http://content.sniklaus.com/github/pytorch-liteflownet/network-kitti.pytorch', file_name='liteflownet-kitti').items() })
    ...
    def forward(self, source_img, target_img):
        ...
        source_img = source_img.cuda().view(-1, 3, int_height, int_width)
        target_img = target_img.cuda().view(-1, 3, int_height, int_width)
