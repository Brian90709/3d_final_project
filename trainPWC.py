...
log_fw = open(f"./logPWC.txt", 'w') # open log file to save log outputs
...
of_dir = './opical-flow-estimation/opticalPWCNet'
# labels as txt file
...
def plot(train_loss,val_loss,title):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("./loss/resultpwc.png")
    plt.show()
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        torch.save(model.state_dict(), f'./pwc{epoch}.pth')
        if(mean_val_loss < best_loss):
            torch.save(model.state_dict(), f'./pwc{epoch}_best.pth')
