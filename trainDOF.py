...
cap = cv.VideoCapture('./speedchallenge/data/train.mp4')
ret, frame1 = cap.read()
...
while(1):
    ...
    np.save('DOF/'+str(count)+'.npy', np.expand_dims(np.transpose(flow, (2, 0, 1)), axis=0))
    prvs = next
    count += 1
...
log_fw = open(f"./logDOF.txt", 'w') # open log file to save log outputs
...
for epoch in range(epochs):
    ...
    history_train_loss.append(mean_train_loss.cpu().detach().numpy())
    log(f'{epoch}t: {mean_train_loss}')
    # validation
    model.eval()
    val_losses = []
    with torch.no_grad():
        ...
        torch.save(model.state_dict(), f'./DEQ{epoch}.pth')
        if(mean_val_loss < best_loss):
            torch.save(model.state_dict(), f'./DEQ{epoch}_best.pth')
            best_loss = mean_val_loss
        history_val_loss.append(mean_val_loss.cpu().detach().numpy())
        log(f'{epoch}: {mean_val_loss}')
